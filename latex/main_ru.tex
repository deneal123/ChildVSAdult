...
(1)~Naturally-supervised (без ручных меток) longitudinal-сигнал, извлечённый из социальных постов.
...
(3)~Взаимоусиливающие контроли (loss-семейство, архитектура, источник, обучающая цель, real-vs-synthetic, диагностика обходного признака), обосновывающие ограниченный тезис: в этом протоколе и среди рассмотренных целей вклад данных в large-gap-прирост больше, чем выбор между loss-семействами.
...
Прирост воспроизводится на FaceNet, ArcFace iResNet и AdaFace IR-50/IR-101 (три loss-семейства, две архитектуры) и \emph{монотонно убывает с силой frozen} (Таблица~\ref{tab:headroom}, Рис.~\ref{fig:headroom}); чистый контроль глубины (AdaFace IR-50 $\rightarrow$ IR-101) это подтверждает. Сильные насыщенные backbone не улучшаются и слегка забывают при дообучении; мягкий learning rate (1e-5) примерно вдвое уменьшает забывание, но не превращает его в прирост. Вклад специфичен для слабых и средних backbone с кросс-возрастным запасом --- ограничение, которое мы указываем явно (разд.~\ref{sec:limitations}).
...
Мы обучаем канонические margin-цели современной FR --- ArcFace-, CosFace- и SphereFace-margin классификацию личности~\cite{deng2019arcface} (ядро OE-CNN/MTLFace) --- на наших naturally-supervised личностях (9\,204 класса), тот же слабый backbone, scope и протокол. На primary endpoint (FG-NET large-gap) контрастив, ArcFace и CosFace \textbf{совпадают} (0.848 / 0.851 / 0.843, в пределах $\pm$0.006; Таблица~\ref{tab:objective}, Рис.~\ref{fig:sota}); лишь SphereFace --- самый трудный для оптимизации margin, здесь без $\lambda$-annealing на 2--3-шот личностях --- отстаёт (0.801). Делаем ограниченный вывод: в этом протоколе и среди рассмотренных целей вклад источника данных доминирует над выбором objective --- а не «метод не важен вообще». ArcFace/CosFace дополнительно \emph{забывают меньше} на easy-бенчмарках --- практическая рекомендация.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{fig_sota_ru.pdf}
\caption{Сравнение objective: контрастив, ArcFace и CosFace совпадают на кросс-возрастном переносе; SphereFace (труднее всего оптимизировать, без $\lambda$-annealing) отстаёт. ArcFace/CosFace чуть меньше забывают на easy-бенчмарках.}
\label{fig:sota}
\end{figure}

\begin{table*}[!t]
\renewcommand{\arraystretch}{1.2}
\caption{Сравнение objective (FaceNet, очищенные данные): контрастив, ArcFace и CosFace совпадают на primary endpoint (FG-NET large-gap); SphereFace (без $\lambda$-annealing) отстаёт на разреженных few-shot личностях.}
\label{tab:objective}
\centering
\footnotesize
\begin{tabular}{@{}lcccc@{}}
...
Сильнейшая внешняя проверка naturally-supervised \emph{источника} --- переносится ли он за пределы исходной платформы. Поэтому мы применяем \emph{идентичный} протокол извлечения к независимому не-VK сообществу --- Reddit-доске r/PastAndPresentPics, чьи «тогда/сейчас» мультифото-посты несут те же текстово-числовые описания возраста, разбираемые той же LLM, --- и оцениваем обученную на VK $+$pairs-модель (без Reddit-данных в обучении) против frozen-базлайна на Reddit-парах (Таблица~\ref{tab:crossplatform}). \fillin{Результат (заполнить по таблице): VK-обученная модель переносится на независимый Reddit-источник, поднимая large-gap ROC-AUC с X до Y, либо отметить наблюдаемую степень переноса.}
\reviewnote{Заготовка до кросс-платформенного прогона (нужны Reddit OAuth-креды). Соберите Reddit-набор под \texttt{ENV\_FOR\_DYNACONF=reddit} (разд.~\ref{sec:repro}: \texttt{scripts/ingest\_reddit.py} $\rightarrow$ preprocess $\rightarrow$ build\_groups $\rightarrow$ cluster\_persons $\rightarrow$ dedup $\rightarrow$ build\_pairs $\rightarrow$ split), затем \texttt{scripts/eval\_cross\_platform.py --ckpts models/bb\_facenet\_seed42.pt --tag vk2reddit} и заполните красные ячейки из \texttt{metrics/cross\_platform\_vk2reddit.json}. Опц.\ \texttt{audit\_ages.py --apply} включает строку large-gap ($\geq$25\,лет).}

\begin{table}[!t]
\renewcommand{\arraystretch}{1.2}
\caption{Перенос между платформами: VK-обученная модель на независимом Reddit (не-VK) кросс-возрастном тесте (frozen против $+$pairs, обучение только на VK). Заполняется после сборки Reddit-набора.}
\label{tab:crossplatform}
\centering
\footnotesize
\begin{tabular}{@{}lcc@{}}
\toprule
Reddit (не-VK) метрика теста & Frozen & $+$pairs (VK) \\
\midrule
Overall ROC-AUC [95\% CI] & \fillin{0.xxx} & \fillin{0.xxx} \\
Large-gap ($\geq$25\,лет) ROC-AUC [95\% CI] & \fillin{0.xxx} & \fillin{0.xxx} \\
EER & \fillin{0.xxx} & \fillin{0.xxx} \\
TAR@FAR$=$1\% & \fillin{0.xxx} & \fillin{0.xxx} \\
\midrule
Тестовых пар (поз / нег) & \multicolumn{2}{c}{\fillin{N / N}} \\
\bottomrule
\end{tabular}
\end{table}
...
Вклад --- масштабируемый naturally-supervised источник longitudinal-супервизии и доказательство того, что он формирует переносимую возрастную инвариантность, которую не заменяет синтетика и не объясняет явная age-супервизия. Взаимоусиливающие контроли --- loss-семейство, архитектура, источник, обучающая цель --- сходятся к ограниченному выводу: в этом протоколе данные доминируют над objective среди рассмотренных вариантов. Диагностика обходного признака даёт чистую identity-only метрику и объясняет механизм; находка чистки (половина сырых позитивов --- дубликаты) сама по себе представляет методологический вклад. Это \emph{data-centric} вклад, а не архитектурный; ряд утверждений ограничен нашей двухисточниковой, одноплатформенной, кажущеся-атрибутной постановкой. Важно: улучшение --- это \emph{целевой} large-gap-прирост, а не drop-in-апгрейд общего верификатора: низко-FAR рабочие точки на easy-бенчмарках деградируют (LFW TAR@FAR$=$0.1\% падает $0.858 \rightarrow 0.575$; Таблица~\ref{tab:stats}), поэтому развёртывание следует ограничивать large-gap кросс-возрастным режимом, а не подменять им модель общего назначения.
...
\noindent\textbf{Не установлено / вне scope.} (a)~эффект \emph{не} является равномерным приростом верификации --- низко-FAR рабочие точки на easy-бенчмарках деградируют (Таблица~\ref{tab:stats}); (b)~сильные насыщенные backbone не улучшаются; (c)~\emph{источник супервизии} проверен только на VK --- внешний child$\leftrightarrow$adult \emph{тест} приведён, но независимый не-VK longitudinal \emph{обучающий} источник остаётся дальнейшей работой; (d)~мы не воспроизводим полный SOTA-конвейер, только его общую margin-цель; (e)~аудит справедливости --- только по кажущимся атрибутам, без human-audited inter-annotator $\kappa$ и полной поячеечной поправки на множественные сравнения (оба --- в release-supplement).
...
\item \textbf{Масштаб и возраст бенчмарков:} мы оцениваем на стандартном для области наборе кросс-возрастных бенчмарков --- AgeDB-30 и CALFW (по 6\,000 пар) и FG-NET для явного подмножества $\geq$25 лет --- тех же, что используют OE-CNN/DAL/MTLFace, что обеспечивает сопоставимость, но наследует их ограниченный масштаб и возраст (FG-NET особенно мал, с частичным покрытием детектором, поэтому headline не опирается только на него). Наш очищенный внутренний тест (5\,107 кросс-возрастных позитивов с контролируемыми негативами) --- крупный современный дополнитель; мы дополнительно приводим child$\leftrightarrow$adult подмножество FG-NET (разд.~\ref{sec:res-fairness}), а больший независимый не-VK \emph{longitudinal}-набор остаётся приоритетом дальнейшей работы.
...
Мультифото-посты --- недорогой, масштабируемый naturally-supervised сигнал кросс-возрастной идентичности; дообучение слабого распознавателя на них даёт переносимый, кросс-источниковый, objective-устойчивый прирост, сосредоточенный на large-gap кросс-возрастном сопоставлении (с лишь незначительным забыванием на easy-бенчмарках), объяснимый снятием возрастного обходного признака, эффективный по данным и не ухудшающий ни одну кажущуюся демографическую страту. Дальнейшая работа: независимый не-VK и child$\leftrightarrow$adult-специфичный внешний набор; human-audited subset супервизии (с agreement человек--LLM); per-point multi-seed доверительные полосы для кривой headroom; полные DeLong-тесты и поячеечная поправка на множественные сравнения; полные SOTA-надстройки; реальная aging-модель на нативном разрешении; и калиброванный демонстратор в рамках governance-ограничений разд.~\ref{sec:ethics}.
