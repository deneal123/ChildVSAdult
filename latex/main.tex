...
(1)~A naturally-supervised (manual-label-free) longitudinal signal mined from social posts---same-person identity pairs, with caption age anchors only a secondary cue.
...
\textbf{Data availability.} Code, configurations, versioned LLM prompts, frozen splits (by hash) and the de-identified benchmark protocol are released; \emph{raw face images and posts are withheld} under the governance constraints of Sec.~\ref{sec:ethics}. We state plainly that this makes replication of the data-mining stage \emph{partial}: pseudonymization does not render face data non-personal, and the LLM caption-parser depends on an external, versioned service subject to temporal drift, so we release its cached outputs alongside the prompts to enable a deterministic replay rather than a live re-run. A controlled-access route to the de-identified derived features for bona-fide researchers is described in Sec.~\ref{sec:ethics}.
