...
\item \textbf{External benchmarks are small/old:} FG-NET has partial detector coverage; the headline is corroborated on AgeDB-30/CALFW, but an independent non-VK longitudinal set and a child$\leftrightarrow$adult-specific set remain desirable (future work).
\item \textbf{Full SOTA pipelines not reproduced end-to-end:} we trained the core ArcFace objective on our source, not method-specific add-ons (MTLFace age synthesis, OE-CNN orthogonal subspaces).
\end{itemize}
...
\reviewnote{The references to CACon, ``Synthetic Face Ageing'' and OrdCon are transcribed from the working draft; verify their authors, venue and year (and replace the corresponding placeholders in \texttt{refs.bib}) before submission.}
