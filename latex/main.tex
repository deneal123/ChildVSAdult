...
Under age-matched negatives (same age bucket, different people), frozen accuracy on the internal 25+ test falls from 0.640 to 0.517 (near the chance level of 0.5), and real pairs restore it to 0.838 (Fig.~\ref{fig:shortcut})---direct evidence that frozen models discriminate cross-age pairs largely by \emph{age}, whereas our pairs induce discrimination by \emph{identity}. A linear probe refines this \emph{representationally} (Table~\ref{tab:leakage}): apparent age stays well decodable from the identity embedding for \emph{all} models ($\approx$0.63--0.65 balanced accuracy $\gg$ 0.25 chance) and changes only slightly, while identity AUC rises sharply---so the method learns an invariant \emph{metric} (the cosine stops relying on the age axis), not an age-free representation; explicit adversarial disentanglement reduces leakage no further.
...
The strongest external check of a naturally-supervised \emph{source} is whether it generalizes beyond the originating platform. We therefore apply the \emph{identical} mining protocol to an independent, non-VK community---the Reddit board r/PastAndPresentPics, whose ``then/now'' multi-photo posts carry the same free-text/numeric age descriptions parsed by the same LLM---and evaluate the VK-trained $+$pairs model (with \emph{no} Reddit data in training) against the frozen baseline on the Reddit-mined cross-age pairs (Table~\ref{tab:crossplatform}). \fillin{Result (fill from the table): the VK-trained model transfers to the independent Reddit source, raising large-gap ROC-AUC from X to Y, or note the observed degree of transfer.}
\reviewnote{Scaffold pending the cross-platform run (needs Reddit OAuth credentials). Build the Reddit set under \texttt{ENV\_FOR\_DYNACONF=reddit} (Sec.~\ref{sec:repro}: \texttt{scripts/ingest\_reddit.py} $\rightarrow$ preprocess $\rightarrow$ build\_groups $\rightarrow$ cluster\_persons $\rightarrow$ dedup $\rightarrow$ build\_pairs $\rightarrow$ split), then run \texttt{scripts/eval\_cross\_platform.py --ckpts models/bb\_facenet\_seed42.pt --tag vk2reddit} and fill the red cells from \texttt{metrics/cross\_platform\_vk2reddit.json}. Optional \texttt{audit\_ages.py --apply} enables the large-gap ($\geq$25\,y) row.}

\begin{table}[!t]
\renewcommand{\arraystretch}{1.2}
\caption{Cross-platform transfer: a VK-trained model evaluated on an independent Reddit (non-VK) cross-age test set (frozen vs.\ $+$pairs, trained on VK only). To be completed once the Reddit set is built.}
...
Large-gap ($\geq$25\,y) ROC-AUC [95\% CI] & \fillin{0.xxx} & \fillin{0.xxx} \\
EER & \fillin{0.xxx} & \fillin{0.xxx} \\
TAR@FAR$=$1\% & \fillin{0.xxx} & \fillin{0.xxx} \\
\midrule
Test pairs (pos / neg) & \multicolumn{2}{c}{\fillin{N / N}} \\
\bottomrule
\end{tabular}
\end{table}
...
\noindent\textbf{Not established / out of scope.} (a)~the effect is \emph{not} a uniform verification gain---low-FAR operating points on easy benchmarks degrade (Table~\ref{tab:stats}); (b)~strong saturated backbones do not improve; (c)~the \emph{supervision source} is validated only on VK---an external child$\leftrightarrow$adult \emph{test} set is reported, but an independent non-VK longitudinal \emph{training} source remains future work; (d)~we do not reproduce a full state-of-the-art pipeline, only its shared margin objective; (e)~the fairness audit uses apparent attributes only and lacks a human-audited inter-annotator $\kappa$ and a full per-cell multiple-comparison correction, both deferred to the release supplement.
