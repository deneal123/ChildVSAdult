...
def main() -> None:
    ...
    # ---- 1. Сколько дисперсии съедает ОХВАТ (на сыром композите e_raw) ---------------
    resid_a, stats_a = tgt.oof_reach_residual(df, splits, variant="A", target="e_raw")
    resid_b, stats_b = tgt.oof_reach_residual(df, splits, variant="B", target="e_raw")
    df["y_symp_A"], df["y_symp_B"] = resid_a, resid_b
    results["reach"] = {"A_exogenous_only": stats_a, "B_also_controls_views": stats_b}

    # ГЛАВНЫЙ таргет: e_rate (ставка на просмотр), очищенный от экзогенного охвата
    # (время/формат/сообщество). views уже поделены внутри e_rate -> reach здесь без views.
    resid_rate, stats_rate = tgt.oof_reach_residual(df, splits, variant="A", target="e_rate")
    df["y_rate"] = resid_rate
    results["rate"] = {
        "loadings": tinfo["rate_loadings"],
        "explained_variance_ratio": tinfo["rate_explained_variance_ratio"],
        "spearman_e_raw_vs_views": tinfo["spearman_e_raw_vs_log_views"],
        "spearman_e_rate_vs_views": tinfo["spearman_e_rate_vs_log_views"],
        "reach_on_rate": stats_rate,
    }

    if args.shuffle_target:
        y = tgt.shuffle_within_bucket(df, df["y_rate"].to_numpy())
        r = mdl.evaluate(df, y, splits, FULL, "y_rate_SHUFFLED", "hgb", args.pca)
        print(json.dumps(r.aggregate(), ensure_ascii=False, indent=2))
        return

    # ---- 2. Ablation-лестница на ГЛАВНОМ таргете (per-view residual) -----------------
    y_a = df["y_rate"].to_numpy()
    ablation = []
    oof_full = None
    for blocks, name in LADDER:
        r = mdl.evaluate(df, y_a, splits, blocks, "y_rate", "hgb", args.pca)
        ...
        if blocks == FULL:
            oof_full = r
    results["ablation_headline"] = ablation

    # ---- 3. Другие таргеты и baseline'ы (для сравнения) -----------------------------
    models: list[dict] = []
    for target_col, tname in [("e_rate", "e_rate (ставка на просмотр)"),
                              ("e_raw", "e_raw (сырой, без views)"),
                              ("y_symp_A", "остаток A (без views)"),
                              ("y_symp_B", "остаток B (views под контролем)"),
                              ("y_pct", "перцентиль ставки в бакете")]:
        r = mdl.evaluate(df, df[target_col].to_numpy(), splits, FULL, tname, "hgb", args.pca)
        models.append(r.aggregate())
    for mname in ["ridge", "dummy"]:
        r = mdl.evaluate(df, y_a, splits, FULL, "y_rate", mname, args.pca)
        models.append(r.aggregate())
    results["models"] = models

    # ---- 4. Негативный контроль -----------------------------------------------------
    y_shuf = tgt.shuffle_within_bucket(df, y_a.copy())
    r_shuf = mdl.evaluate(df, y_shuf, splits, FULL, "y_rate_SHUFFLED", "hgb", args.pca)
    ...
    out = pd.DataFrame({
        ...
        "views": df["views"], "e_raw": df["e_raw"], "e_rate": df["e_rate"],
        "y_rate": y_a, "pred_rate": oof_full.oof_pred,
    ...
    print(json.dumps({
        "HEADLINE_target": "y_rate (per-view content beyond reach)",
        "E_raw_vs_views_spearman": tinfo["spearman_e_raw_vs_log_views"],
        "E_rate_vs_views_spearman": tinfo["spearman_e_rate_vs_log_views"],
        "content_on_rate": {k: o[k] for k in ["r2", "spearman", "ndcg@50", "lift@50"]},
